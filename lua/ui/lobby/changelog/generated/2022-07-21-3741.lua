...
--                                                 Source: docs/_posts/2022-07-21-3741.md                                                 --
--------------------------------------------------------------------------------------------------------------------------------------------
Changelog = {
    version = "2022-07-21-3741",
    description = {
        "# Game version 3741 (21st of August, 2022)",
        "",
        "Another month, another patch! With pride we present the next version of the game where we slowly start tackling long-standing issues that",
        "we've been taking for granted. Among them the tactical missile defenses that feel underwhelming, broken weapons such as those of the",
        "Galactic Colossus and beam weapons being unable to hit reliable. But soon no more - we're tackling them one by one!",
        "",
        "Not only that, from a development perspective we've made great strides. The amount of annotated code has exploded, allowing our extension to",
        "give proper suggestions and hints. Step by step, pull request by pull request we slowly turn the code base into one that supports modern",
        "programming features such as being able to jump to a definition and we now provide sane suggestions and snippets.",
        "",
        "Let alone the improvements to the infrastructure surrounding FAF. With the cache servers of Cloudflare as our backbone the distribution of",
        "maps, mods and game assets have never been better.",
        "",
        ...
        "",
        "  A complete re-implementation of how the game results are tracked. This should fix the famous draw bug (where one player wins and the other",
        "  loses) and in general there should be less unknown game results.",
        "",
        ...
        "",
        "  Due to how the game processes the game over several steps the area damage could be applied too late, allowing really fast units to already",
        "  be out of the area of damage, even though the projectile had a direct impact. This is particularly noticeable for spy planes, where they",
        ...
        "",
        "  Due to how the game processes beams there's a possibility that the beam aims 'in front' of the unit, causing it to miss. This is fixed by",
        "  giving the unit (air units, for this change) a collision box (or sphere) that is of sufficient size. As a result, the Seraphim Lighting",
        "  Tank (T3 MAA) no longer requires its hack, and because of that can no longer pierce shields :).",
        "",
        ...
        "",
        "  Allows their cargo to receive (splash) damage more consistently and makes the transports easier to hit when they are landed.",
        "",
        "- (#4082, #4083, #4084) Fix target bones - Seraphim Land units",
        "",
        "  Similar to #4058 and #4063, makes these units easier and more consistent to hit when firing with beam weapons. We'll slowly tackle all of",
        "  the units in the base game, but this is a consuming process!",
        "",
        ...
        "",
        "  Long story short - we tried applying some tricks to improve performance a while ago and those did not work anymore after saving and then",
        "  loading in the game. As a consequence, after loading the game was working with the wrong data causing all sorts of issues!",
        "",
        "  We hope this fixes it, but if you still have issues with saving and loading then you can contact us via Discord or the forums.",
        "",
        "- (#4113) Reduce projectile speed of air to air weapons",
        "",
        "  There's a collision bug that causes the game engine to pick the furthest collision if a projectile collides with multiple instances at the",
        "  same time. As a consequence, it was difficult for interceptors to hit landed air units and in particular transports. By reducing the",
        "  projectile speed (and increasing the size of the hitboxes with #4078) we try and solve this issue consistently for the base game",
        "",
        ...
        "",
        "- (#4140, #4152) Fix rare occasions when the status quo of HQs are not updated properly In particular, when gifting due to full share and",
        "  when directly destroying the unit",
        "",
        ...
        "",
        "  Re-implements the tractor beams of the Galactic Colossus from the ground up. They finally work as intended and can no longer remain stuck",
        "  on invalid targets. Includes a slight boost to the effects, enjoy toying with the physics!",
        "",
        "- (#3893) Add shooter cap to tactical missiles",
        "",
        "  Prevents the TMDs from overshooting their targets, as a result TMDs feel a lot more consistent and are in general a lot stronger.",
        "",
        ...
        "",
        "  Improves the mod manager by introducing a search bar, being able to mark mods as favorite and being able to sort mods on their type.",
        "  Introduces the capability of interpreting URLs from the `mod_info.lua` file, allowing you to send people to a github, gitlab, bitbucket,",
        "  sourceforge page or to a topic on the forums.",
        "",
        "- (#3927) Allow jammers to recreate their jamming blips",
        "",
        "  Fixes the issue of jamming blips being gone forever once they were scouted. With this implementation, after fifteen seconds of being in",
        "  the fog of war the jamming blips should re-emerge on their own.",
        "",
        "- (#4114, 44fd74) Alternative CPU benchmark pt. 1",
        "",
        "  The first of two parts on creating a new CPU benchmark. The current benchmark tries to run some computations in the lobby. This has shown",
        "  to be inaccurate. The new benchmark takes samples during the game and stores the highest possible sim rate you can run in combination with",
        "  the total amount of units. Initial data points suggest that this is highly accurate and is even capable of detecting the throttling of",
        "  laptops.",
        "",
        "  The next part will replace implementation of the CPU score in the lobby. That will be part of the next release. This guarantees that",
        "  everyone has some data to start off with.",
        "",
        ...
        "",
        "  The game now keeps track of the reclaim statistics of engineers. They are shown where you'd usually see the veterancy of a unit. You can",
        "  not see the statistics of allied units, just your own. Works for observers (and casters).",
        "",
        "  This is the start of a new era where we'll be introducing statistics gradually. They help players with understanding the value of their",
        "  actions - as an example, an engineer reclaiming trees on Seton's Clutch can quickly become responsible for hundreds (if not thousands) of",
        "  additional mass.",
        "",
        "## Performance",
        "",
        ...
        "",
        "  Significantly reduces the amount of table allocations when we correct the trajectory of a bomber in Lua.",
        "",
        ...
        "",
        "  When the patrol path was generated, there was a logic to move instead of patrol if the reclaiming was disabled on the base. That got",
        "  removed since just moving engineers around does nothing. So it's either patrol or nothing.",
        "",
        "- (26bcee) Fix base manager rebuild counter",
        "",
        "  Rebuild structure counter was decremented even for spawning units. Meaning that that AI would not rebuild at all, while it was suppose to",
        "  be rebuilding once.",
        "",
        "- (fa1448) Update base manager rebuild data",
        "",
        "  Used for counting how many times to rebuild stuff, based on difficulty. Some of the names were wrong and the list was kinda incomplete.",
        "",
        "- (8e167b) Fix base manager sometimes not upgrading factories",
        "",
        "  Upgrade thread was checking for IdleState, but if the factory that was suppose to be upgraded was grabbed first to assist other factories",
        "  in the base, it would never be idle.",
        "",
        ...
        "",
        "  Prevents them from wandering off, taking on quests - finding loot and end up dying because they're on their own chasing their dreams.",
        "",
        ...
        "",
        "  This is a difficult one to tackle, but what happens is that buildings remain unfinished because there is a nearby threat to the ACU. The",
        "  ACU attempts to defend, but then doesn't always continue what he started previously",
        "",
        ...
        "",
        "- (#4070, #4100) Add unit utilities Step-up work to make sacrifice work properly again",
        "",
